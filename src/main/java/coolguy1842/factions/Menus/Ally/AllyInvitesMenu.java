...
public class AllyInvitesMenu implements Menu {
    ...
    public void loadInventory(FactionPlayer p) {
        ...
        Inventory menu = Bukkit.createInventory(null, 54, Component.text("Ally Invites Menu"));
        ...
        for(Faction f : FactionsManager.getInstance().factionManager.factions.values()) {
            if(!pFaction.hasAllyInvite(f) || pFaction.isAllied(f) || f.getID().equals(pFaction.getID())) continue;
