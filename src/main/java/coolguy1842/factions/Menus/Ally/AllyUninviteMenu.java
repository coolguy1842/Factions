...
public class AllyUninviteMenu implements Menu {
    ...
    public void loadInventory(FactionPlayer p) {
        ...
        Inventory menu = Bukkit.createInventory(null, 54, Component.text("Ally Remove Invite Menu"));
        ...
        for(Faction f : FactionsManager.getInstance().factionManager.factions.values()) {
            if(!f.hasAllyInvite(pFaction) || f.getID().equals(pFaction.getID())) continue;
    ...
    public void onClick(InventoryClickEvent e, FactionMenu menu) {
        if(e.getCurrentItem().equals(items[AllyInviteMenuItems.BACK.ordinal()])) {
            ...
            menu.getPlayer().getFaction().removeAllyInvite(FactionsManager.getInstance().factionManager.getFaction(allyItems.get(e.getSlot())));
